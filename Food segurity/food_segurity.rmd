...
nutrition <- readr::read_csv(
    "c:/Users/italo/OneDrive/Área de Trabalho/VsCode/faostats/FAOStats Suite of Food Security Indicators.csv") |> # nolint
...
nutrition_new <- nutrition |>
    dplyr::filter(
        item == "Average dietary energy requirement (kcal/cap/day)" # Minimum dietary energy requirement (kcal/cap/day)
    ) |>
    dplyr::mutate(
...
title_text <- glue::glue("Disponibilidade energética média da dieta humana")
caption_text <- glue::glue("**Dados** FAOStats (ano 2022) **| Gráfico** Ítalo Monteiro") # nolint
...
```{r}
ggsave("Food segurity/NecessidadeMediaEnergiaDierta.png", plot = p3b, dpi = 600)
```